...
class PublicServer:
    ...
    def start(self):
        ...
        self.environment.update(
            GENRO_ASGI_HOME=str(self.directory / "home"), TMPDIR=str(self.directory)
        )
...
def test_complete_configuration_readback(recipes, tmp_path):
    ...
    code = recipes.get_code("guides/configuration.md", "from genro_asgi import AsgiServer")
    site = tmp_path / "site"
    site.mkdir()
...
'''
    ...
    environment.update(
        GENRO_ASGI_HOME=str(tmp_path / "home"), TMPDIR=str(tmp_path),
