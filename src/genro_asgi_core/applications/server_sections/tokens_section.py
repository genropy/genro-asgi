...
if __name__ == "__main__":
    ...
    class _Server:
        api_key_store = _Store()
        auth_core = AuthCore(jwt=[{"secret": "sign-me", "algorithm": "HS256", "name": "main"}])

    class _App:
        server = _Server()

    section = TokensSection(_App())
    issued = section.issue({"label": "ci-bot", "tags": ["ci"]})
    assert issued["key"].startswith("gak_")
    listed = section.list()["tokens"]
    assert all("secret_hash" not in r for r in listed)  # never on the wire
    key_id = listed[0]["key_id"]
    assert section.revoke(key_id)["revoked"] is True
    minted = section.create_jwt({"sub": "robot", "tags": ["worker"], "expires_in": 60})
    decoded = _Server.auth_core.authenticate(
        {"headers": [(b"authorization", b"Bearer " + minted["token"].encode())]}
    )
    assert decoded is not None and decoded.identity == "robot" and decoded.tags == ["worker"]
    assert section.create_jwt({"tags": []})["error"]  # sub required

    class _ServerNoJwt:
        api_key_store = _Store()
        auth_core = AuthCore()  # no jwt verifier configured

    class _AppNoJwt:
        server = _ServerNoJwt()

    assert TokensSection(_AppNoJwt()).create_jwt({"sub": "x"})["error"]  # no jwt config → error
