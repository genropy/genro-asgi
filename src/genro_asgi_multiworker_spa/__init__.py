...
It is a package of its own, beside `genro_asgi` and shipped with it:
``import genro_asgi`` loads none of this, and this side imports the core
by absolute path. Everything here is inert until a runtime (or
genropy-asgi) mounts the front. This ``__init__`` is the public face of
the registers (``from genro_asgi_multiworker_spa import RegisterRegistry``);
