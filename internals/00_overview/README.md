...
Coarse on purpose: it lists and points, the detail lives in each folder.

```mermaid
flowchart TB
    subgraph HZ ["Horizons — 🔴 unratified proposals"]
        DEP["150 deployment-bundles"]
        K8S["160 kubernetes-deploy"]
        SUB["170 subcommanders"]
    end
    subgraph SPAW ["The SPA world"]
        SPA["090 spa-application"]
        ORC["100 orchestration"]
        GS["110 global-store"]
        DC["120 datachanges"]
        DB["130 dbevents"]
        RS["140 restart"]
    end
    subgraph ADM ["Running and administering"]
        CFG["010 configuration"]
        CLI["015 cli"]
        AUTH["020 authentication"]
        SRV["030 server-application"]
        MON["040 monitor"]
        INS["050 inspector"]
        CON["060 console"]
        TSK["070 tasks"]
    ...
    subgraph LAY ["Layers — the shelves"]
        CH["010 channel"]
        ST["020 storage"]
        MW["030 middleware"]
        RT["040 routing"]
