...
class PublisherDispatcher(Dispatcher):
    ...
    def _get_apps(self) -> dict[str, RoutedClass]:
...
class AsgiPublisher(AsgiServer):
    ...
    Adds:
        - mount() for mounting RoutedClass apps at paths
    ...
    def mount(self, path: str, app: RoutedClass) -> None:
        """
        Mount a RoutedClass application at a path.

        Args:
            path: Mount path (e.g., "/api", "/shop"). Must be unique.
            app: RoutedClass instance to mount.
