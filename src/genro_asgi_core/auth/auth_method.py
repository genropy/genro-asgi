...
outcome: an identity-bearing session issued through ``server.promote_session``
— the cookie for the change is emitted by ``SessionMiddleware`` (option A),
never by a method or a handler.
