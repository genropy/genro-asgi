...
As of the 2026-09-08 documentation audit, the repository had no homepage URL,
no RTD URL in tracked documentation, and no RTD check/status on the audited
`develop` commit. The canonical RTD project and its latest remote build remain
unverified. A maintainer must confirm the project URL, repository integration
and version/branch settings before publication is claimed. Successful local
builds do not establish remote publication.
