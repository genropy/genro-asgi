...
layered `config.py` under `GENRO_ASGI_HOME`, environment variables; the
`genroasgi` CLI that drives it is its own feature (`015_cli`). Every other feature reads its own subtree of the recipe; an
