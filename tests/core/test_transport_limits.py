...
from genro_asgi.channel.frame import Frame, FrameCodec, FrameStream, HEADER, CHANNEL_MAGIC, CHANNEL_VERSION
from genro_asgi.channel.local import LocalFrameStream
...
def test_environment_policy_and_explicit_override(monkeypatch):
    monkeypatch.setenv("GNR_ASGI_FRAME_MAX_BYTES", "2097152")
    monkeypatch.setenv("GNR_ASGI_FRAME_WARN_BYTES", "12345")
    monkeypatch.setenv("GNR_ASGI_FRAME_WARN_INTERVAL_SECONDS", "7")
    codec = FrameCodec()
    ...
    monkeypatch.setenv("GNR_ASGI_HTTP_MAX_BODY_BYTES", "1000")
    assert HttpRecord().max_body_size == 1000
    assert BufferedAsgiEndpoint(AsyncMock()).max_body_size == 1000


@pytest.mark.parametrize("name,value", [
    ("GNR_ASGI_FRAME_MAX_BYTES", "0"), ("GNR_ASGI_FRAME_MAX_BYTES", "many"),
    ("GNR_ASGI_FRAME_MAX_BYTES", str(2**32)), ("GNR_ASGI_FRAME_WARN_BYTES", "-1"),
    ("GNR_ASGI_FRAME_WARN_INTERVAL_SECONDS", "-1"),
])
...
def test_warning_threshold_throttling_and_no_payload(caplog, monkeypatch):
    ...
    monkeypatch.setattr("genro_asgi.channel.frame.time.monotonic", lambda: 100)
    wire = codec.encode(frame)
    codec.encode(frame)
    assert len(caplog.records) == 1
    assert "worker-1" in caplog.text and "path=/large" in caplog.text
    assert "PRIVATE-PAYLOAD" not in caplog.text
    monkeypatch.setattr("genro_asgi.channel.frame.time.monotonic", lambda: 160)
...
async def test_remote_runner_oversized_result_is_a_reply_error_and_next_call_succeeds():
    from genro_asgi.remote_runner import RemoteApplicationRunner
...
async def test_spa_oversized_http_request_returns_413_before_dispatch(monkeypatch):
    ...
    monkeypatch.setenv("GNR_ASGI_HTTP_MAX_BODY_BYTES", "1024")
